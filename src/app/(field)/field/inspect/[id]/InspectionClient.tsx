                      ...
                      <div key={item.id} className="border-b border-[#27272a] last:border-b-0">
                        <div className="p-4">
                          ...
                          <div className="grid grid-cols-4 gap-2 mb-3">
                            {(['pass', 'fail', 'attention', 'na'] as const).map(status => (
                              <button
                                key={status}
                                ...
                                className={cn(
                                  ...
                                  item.status === status
                                    ? status === 'pass' ? 'bg-green-500/10 border-green-500 text-green-500'
                                    : status === 'fail' ? 'bg-red-500/10 border-red-500 text-red-500'
                                    : status === 'attention' ? 'bg-yellow-500/10 border-yellow-500 text-yellow-500'
                              ...
                              >
                                ...
                                {status === 'attention' && <AlertTriangle className="w-5 h-5" />}
                                {status === 'na' && <span className="text-lg font-bold">—</span>}
                                {status === 'pass' ? 'Pass' : status === 'fail' ? 'Fail' : status === 'attention' ? 'Attn' : 'N/A'}
                              </button>
                            ))}
                          </div>
